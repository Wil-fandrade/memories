...
export default function CartPage() {
  const [quantity, setQuantity] = useState(() => {
    ...
    const storedQuantity = window.localStorage.getItem("memories-cart-quantity");
    return storedQuantity ? Math.max(1, Number(storedQuantity)) : 1;
  });
  const product = catalogProducts[0];
  const total = 34 * quantity;
  useEffect(() => {
    window.localStorage.setItem("memories-cart-quantity", String(quantity));
  }, [quantity]);
